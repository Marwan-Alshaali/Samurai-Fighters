...
//params: x pos, y pos, width, height
	...
	if(!player.dead) {
		switch (event.key) {
			...
			case 'a':
				...
				break
			case 'w':
				if(player.position.y + player.height + player.velocity.y >= canvas.height - 93) {
					player.doubleJump = 0
				}
	...
	if(!enemy.dead) {
		switch (event.key) {
			...
			case 'ArrowLeft':
				...
				break
			case 'ArrowUp':
				if(enemy.position.y + enemy.height + enemy.velocity.y >= canvas.height - 93) {
					enemy.doubleJump = 0
				}
				if(enemy.doubleJump <= 1) {
					enemy.velocity.y = -8
					enemy.doubleJump++
				}
				break
